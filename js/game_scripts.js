...
socket.on("user turn",(data)=>{
  socket.emit("user turn",data)
})

socket.on("show word",(data)=>{
  document.getElementById("word").innerHTML = data.word;
  socket.emit("show data to other players",data)
})

socket.on("show data to other players",(data)=>{
    var word = data.word
    var dashes = word.replace(/./g, '_ ');
    document.getElementById("word").innerHTML = dashes;
    document.getElementById("curr_player").innerHTML = data.curr_player;
})
...
socket.on("update timer",(data)=>{
  // update_operation({word:data.word, curr_user:data.username})
  console.log(data.timeleft)
  document.getElementById("timer").value = 20 - data.timeleft;
